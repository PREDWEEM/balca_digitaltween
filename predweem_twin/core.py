"""Núcleo biofísico de PREDWEEM adaptado a Balcarce vK4.9.18.

Conserva las entradas ANN y los filtros del repositorio LOLIUM_BAL2026 e
incorpora explícitamente el agotamiento post-pico de la cohorte. Se separó de
Streamlit para que pueda probarse, auditarse y reutilizarse dentro del gemelo.
"""
...
class ModelParameters:
    ...
    decay_tau_days: float = 3.5656
    decay_beta: float = 0.48684
    decay_intensity: float = 0.95
    cohort_exhaustion_days: int = 110
    cohort_remaining_threshold: float = 0.005
...
def apply_cohort_decay(
    ...
    """Aplica el agotamiento Weibull post-pico calibrado para Balcarce.

    La intensidad mezcla la curva calibrada con ausencia de agotamiento. La
    extinción fisiológica evita que lluvias tardías reactiven una cohorte cuyo
    remanente Weibull ya es despreciable.
    """
    df = trajectory.copy()
    ...
    df["Cohorte_Agotada"] = False
    df["Criterio_Agotamiento_Dias"] = int(params.cohort_exhaustion_days)
    df["Criterio_Remanente_Maximo"] = float(
        params.cohort_remaining_threshold
    )

    if first_peak_index is None or not params.decay_enabled:
        return df

    tau = max(float(params.decay_tau_days), 0.01)
    beta = max(float(params.decay_beta), 0.01)
    intensity = float(np.clip(params.decay_intensity, 0.0, 1.0))
    peak_date = pd.Timestamp(df.at[first_peak_index, "Fecha"])
    days_from_peak = (
        pd.to_datetime(df["Fecha"]) - peak_date
    ).dt.days.clip(lower=0).astype(float)
    before_peak = pd.to_datetime(df["Fecha"]) < peak_date

    base_factor = np.exp(-np.power(days_from_peak / tau, beta))
    base_factor = np.where(before_peak, 1.0, base_factor)
    applied_factor = 1.0 - intensity * (1.0 - base_factor)
    exhausted = (
        (days_from_peak >= float(params.cohort_exhaustion_days))
        & (base_factor <= float(params.cohort_remaining_threshold))
        & ~before_peak
    )
    applied_factor = np.where(exhausted, 0.0, applied_factor)

    df["Dias_Desde_Primer_Pico"] = days_from_peak
    df["Factor_Decaimiento_Base"] = base_factor
    df["Factor_Decaimiento"] = applied_factor
    df["Cohorte_Agotada"] = exhausted
    df["EMERREL"] = np.clip(
