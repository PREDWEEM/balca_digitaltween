...
def saved_profile():
    return load_site_profile(DATA / "balcarce_2026.json")


def apply(frame=None, **kwargs):
    arguments = {
        "site": "Balcarce", "as_of": "2027-01-08",
...
def test_invalid_profile_cannot_change_outputs():
    ...
    with pytest.raises(ValueError, match="finitos"):
        apply_site_calibration(example_trajectory(), profile, site="Balcarce", as_of="2027-01-08")
...
def real_data():
    ...
    raw = pd.read_csv(DATA / "balcarce_2026_counts.csv")
...
def test_source_units_missing_replicates_and_irregular_intervals(real_data):
    ...
    assert len(raw) == 18 and "n_repeticiones" not in metadata
    assert raw.columns.tolist() == ["FECHA", "PLM2"]
    assert raw.PLM2.sum() == pytest.approx(8280.)
    _, intervals = calibration_intervals(trajectory, prepared)
    assert intervals.iloc[0]["Inicio_exclusivo"] == pd.Timestamp("2026-03-12")
    assert intervals.iloc[0]["Fecha"] == pd.Timestamp("2026-03-20")
    assert intervals.iloc[0]["Dias_intervalo"] == 8
    assert intervals.iloc[0]["Observado_PLM2"] == pytest.approx(6300.)
    assert intervals.loc[intervals.Fecha.eq("2026-07-03"), "Dias_intervalo"].iloc[0] == 14
    ...
    assert intervals.Sigma_ajuste_PLM2.eq(630.).all()
    assert intervals.Observado_PLM2.sum() == pytest.approx(raw.iloc[:, -1].sum())


def test_calibration_keeps_balcarce_decay_exhaustion_and_local_reference(real_data):
    trajectory, _, _, _ = real_data
    result, audit = apply(trajectory, as_of="2026-08-15")
    assert audit["applied"]
    for column in ("EMERREL", "Factor_Decaimiento", "Cohorte_Agotada", "TT_DESDE_PICO"):
        pd.testing.assert_series_equal(trajectory[column], result[column])
    exhausted = result.Cohorte_Agotada
    assert exhausted.any()
    assert result.loc[exhausted, "EMERREL_CALIBRADA"].eq(0).all()
    saved = saved_profile()
    assert saved["model_parameters"]["cobertura_pct"] == 10.
...
def test_fit_matches_persisted_profile_and_does_not_mutate_network(real_data):
    ...
    profile, comparison = fit_site_calibration(trajectory, prepared, site="Balcarce")
    saved = saved_profile()
...
def test_invalid_training_data_are_rejected(real_data, fault):
    ...
    with pytest.raises(ValueError):
        fit_site_calibration(trajectory, obs, site="Balcarce")
