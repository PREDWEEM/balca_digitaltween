...
def test_two_campaign_percentiles_have_equal_individual_weight():
    ...
    assert reference.loc[reference.Julian_days.lt(71), "N_Campanas_Dia"].eq(1).all()
...
def test_2026_total_is_unavailable_before_last_visit_even_if_future_counts_change(local_files):
    ...
    assert earlier.N_Campanas.eq(1).all()
    ...
    assert available.N_Campanas.eq(2).all()
...
def test_excluded_campaigns_cannot_change_the_local_pool_or_intensity_reference(local_files):
    ...
    with (local_files / MODEL).open("rb") as handle:
        payload = pickle.load(handle)
    selected = {"emererel2025 balcarce.xlsx" for year in YEARS}
...
def test_explicit_whitelist_and_exclusions_are_auditable():
    reference = load_local_seasonal_reference(ROOT, "2027-05-05")
    assert reference.Campanas.iloc[0] == "emererel2025 balcarce.xlsx, balcarce_2026_counts.csv"
