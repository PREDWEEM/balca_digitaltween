# -*- coding: utf-8 -*-
# ===============================================================
# 🌾 NODO CLIMÁTICO PREDWEEM — BALCARCE
#
# Serie operativa:
#   • Fechas observadas: SIGA–INTA Balcarce (fuente prioritaria).
...
LATITUD = float(os.getenv("LATITUD", "-37.7664"))
LONGITUD = float(os.getenv("LONGITUD", "-58.2999"))
...
ARCHIVO_SIGA_CACHE = Path("data/siga_balcarce_observado.csv")
DIRECTORIO_PRONOSTICOS = Path("data/historico_pronosticos")
ARCHIVO_ESTADO = Path("data/estado_actualizacion_meteo.json")

SIGA_ARCHIVO_LOCAL = Path(os.getenv("SIGA_LOCAL_FILE", "A872824.xls"))
...
def descargar_siga(fecha_inicio: date, fecha_fin: date) -> tuple[bytes, str, str]:
    ...
    nombre = coincidencia.group(1) if coincidencia else Path(url).name or "siga_balcarce.xls"
...
def normalizar_dataframe_siga(
    ...
    salida["Fuente"] = "SIGA_INTA_BALCARCE"
...
def obtener_siga_dataframe(
    ...
    if SIGA_URL_TEMPLATE and archivo_forzado is None:
        try:
            print("📡 Descargando observaciones diarias SIGA Balcarce...")
    ...
    if ARCHIVO_SIGA_CACHE.exists():
        try:
            print("📦 Utilizando caché observado de SIGA Balcarce.")
...
def cargar_pronostico_ecmwf() -> pd.DataFrame:
    ...
    archivo = (
        ...
        / f"ecmwf_ifs_ens_025_balcarce_{marca}.csv"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Actualiza meteo_daily.csv para PREDWEEM Balcarce."
    )
    parser.add_argument(
        "--output",
