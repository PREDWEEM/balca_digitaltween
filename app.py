"""Interfaz PREDWEEM Digital Twin — Balcarce."""
...
st.markdown(
    ...
    <div class="hero">
      ...
      <h1>Gemelo Digital · Lolium Balcarce</h1>
...
with st.expander("Configuración del gemelo", expanded=True):
    ...
    with weather_column:
        ...
        source_option = st.radio(
            "Meteorología",
            ["SIGA Balcarce + ECMWF operativa", "Open-Meteo", "Cargar archivo"],
        )
        ...
        coverage = st.slider(
            "Cobertura constante o de respaldo (%)", 0, 100, 10, 5
        )
        coverage_notice = st.empty()
    with parameter_column:
        st.markdown("**Parámetros del gemelo**")
        w_max = st.slider("Agua superficial Wmax (mm)", 5.0, 36.0, 10.0, 0.5)
    ...
    st.markdown("**Perfil concentrado Balcarce v1**")
    st.caption(
        "Latencia JD 25 · decaimiento Weibull τ=3,5656 días, "
        "β=0,48684 e intensidad 0,95 · extinción fisiológica a 110 días."
    )
...
try:
    calibration_profile = load_site_profile(CALIBRATION_DIR / "balcarce_2026.json")
...
if source_option == "SIGA Balcarce + ECMWF operativa":
    st.caption(
        "Meteorología: observaciones SIGA–INTA Balcarce, puente histórico ECMWF "
...
st.caption(
    f"Pool de referencia de Balcarce: {reference_years} "
...
with tab_state:
    ...
    st.toggle(
        ...
        help=(
            ...
            "El pool histórico local permanece activo. "
    ...
    with left:
        ...
        st.write(
            ...
            f'y la cohorte está **{"agotada" if snapshot["cohort_exhausted"] else "disponible"}**.'
        )
        st.info(
...
with tab_calibration:
    st.subheader("Memoria local de Balcarce · campaña 2026")
    st.info(calibration_audit["reason"])
    if calibration_profile is not None:
        fit = calibration_profile["fit"]
        source = calibration_profile["source"]
        fit_table = pd.read_csv(CALIBRATION_DIR / "balcarce_2026_fit.csv")
        st.write(
        ...
        st.download_button(
            ...
            (CALIBRATION_DIR / "balcarce_2026_counts.csv").read_bytes(),
            "balcarce_2026_counts.csv", "text/csv",
        )
        st.caption(
            'Los conteos integran el pool histórico y la calibración local. Para asimilarlos '
...
with tab_audit:
    ...
    with st.expander("Curvas de la referencia local"):
        ...
        st.download_button(
            ...
            "balcarce_referencia_local.csv", "text/csv",
        )
    ...
    audit_summary = pd.DataFrame(
        {
            "Variable": [
                ...
                "Decaimiento post-pico", "Referencia estacional local",
                "Cohorte agotada al corte",
            ],
            "Valor": [
                ...
                "Balcarce concentrado v1",
                (
                    f"Weibull: tau={parameters.decay_tau_days:.4f} d; "
                    f"beta={parameters.decay_beta:.5f}; "
                    f"intensidad={parameters.decay_intensity:.2f}"
                ),
                f"Balcarce {reference_years}; n={reference_campaigns} campaña(s)",
                "Sí" if snapshot["cohort_exhausted"] else "No",
            ],
        }
    )
    ...
    export_columns = [
        ...
        "Dias_Desde_Primer_Pico", "Factor_Decaimiento_Base",
        "Factor_Decaimiento", "Cohorte_Agotada",
        "Criterio_Agotamiento_Dias", "Criterio_Remanente_Maximo",
    ]
    export_columns = [
